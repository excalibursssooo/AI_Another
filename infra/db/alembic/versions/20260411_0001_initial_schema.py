...
def upgrade() -> None:
    op.execute(
        ...
        CREATE INDEX IF NOT EXISTS idx_feed_post_user
        ON feed_post (user_id, created_at DESC);

        CREATE INDEX IF NOT EXISTS idx_task_item_user_created
        ON task_item (user_id, created_at DESC);

        CREATE INDEX IF NOT EXISTS idx_audit_log_ts
        ON audit_log (ts DESC);

        CREATE INDEX IF NOT EXISTS idx_agent_profile_domain_status_updated
        ON agent_profile (domain_id, status, updated_at DESC);
        """
    )
